...
class TokenGroupCounter:
    ...
    def iterate(self):
        ...
        top_pairs = [pair for pair, freq in most_common_pairs if freq == top_frequency]
...
def pre_processing(initial_str: str, special_tokens: list[str]) -> Counter[str]:
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    with open(input_path, "rb") as f:
        ...
        number_of_runs = len(intervals) // num_processes

        if len(intervals) % num_processes != 0:
            number_of_runs += 1
        ...
        for i in range(number_of_runs):
            if i % 10 == 0:
                print("chunk ", i)
            chunks = []
            cur_run_start = i * number_of_runs
            cur_run_end = min((i + 1) * num_processes, len(intervals))
            for start, end in intervals[cur_run_start:cur_run_end]:
                f.seek(start)
